...
這個 agent 的目標是協助行銷人員檢查 `data/` 內的行銷資料品質，包含 raw platform data 與合併後的 `360.csv`。

行銷人員可以透過維護各 skill 底下的 `references/rules.md`，把業務經驗、異常判斷標準、命名規則與檢查邏輯文本化。Claude Code、Codex 或 Gemini CLI 執行時，應讀取這些規則，檢查資料，並輸出 markdown 報告。
